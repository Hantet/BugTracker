...
function addscreenname(full,mini)
{
	var screen = document.getElementById("screen_links").innerHTML;
	document.getElementById("screen_links").innerHTML = screen + full + '^' + mini + '*';
}
...
function stopUpload(html)
{
	...
	if(html)
	{
		...
		if(exp[0] == '2')
		{
			res.innerHTML += '<div align="center"><a href="screen/'+exp[3]+'" target="_blank"><img src="screen/'+exp[1]+'"></a></div><div align="center"><small><font color="#FFFFFF">Размер: '+exp[2]+' Кб</font></small></div>';
			addscreenname(exp[3],exp[1]);
		}
		else
			alert(exp[1]);
	}
...
function DeleteComment(id,detail)
{
	window.location.href = 'index.php?a=list&detail='+detail+'&delcomm='+id;
}
...
function addcomment(entry,admin)
{
	if(admin == 0)
	{
	...
	{
		var text = document.getElementById("admin_reply").value;
		var name = document.getElementById("comm_admin").options[document.getElementById("comm_admin").selectedIndex].value;
		var nametxt = document.getElementById("comm_admin").options[document.getElementById("comm_admin").selectedIndex].text;
		var account = document.getElementById("userid").innerHTML;
		var query = "player="+name+"&text="+text+"&account="+account+"&entry="+entry+"&admin=1";
	}
	if(name == 0)
		...
		$.ajax({
			...
			success: function(html){
				if(admin == 0)
				{
					...
					document.getElementById("textarea2").innerHTML = '<div class="pad2">'+html+' ['+nametxt+']:<div class="pad2">'+htmlspecialchars(text)+'</div></div><hr>' + old;
					document.getElementById("comm_text").value = '';
					showhide0("commentlayer");
				}
				else if(admin == 1)
				{
					var old = document.getElementById("textarea3").innerHTML;
					document.getElementById("textarea3").innerHTML = '<div class="pad2">'+html+' ['+nametxt+']:<div class="pad2">'+htmlspecialchars(text)+'</div></div><hr>' + old;
					document.getElementById("comm_admin").value = '';
					showhide0("adminlayer");
					showhide0("textarea3");
				}
			}
		});
...
function send(player,type,subtype,map,zone,title)
{
	...
	var screens = document.getElementById("screen_links").innerHTML;
	var i = 0;
	var j = 0;
	var diff = 0;
	var links = '';
	...
	if(area1.length > 0 && area2.length > 0 && i > 0)
	{
		$.ajax({
			type: "POST",
			data: "a1="+area1+"&a2="+area2+"&a3="+area3+"&userid="+userid+"&sql="+sql+"&links="+links+"&countscreen="+j+"&screens="+screens,
			url: "send.php",
			cache: false,
			success: function(html)
			{
